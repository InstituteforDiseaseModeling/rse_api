...
class SimpleController(MethodView):
    def __init__(self, model, many_schema, single_schema=None,
        ...
        :param many_schema: Marshmallow schema class that represent the List view of the model. Sometimes you may
        want to disable nested objects in the list view but most of the time there is one schema for both single and many
        :param single_schema: Schema for display of single objects and also for input. If not specified, the many
        schema will be used
        :param exclude_on_post: Fields to exclude on post
        :param order_by: Field to order list by default
        """
        super().__init__()
        # We assume that DB has been loaded before any controllers. Otherwise, some application specific setup could be
        # missed
