...
class SwaggerSpec:
    ...
    def set_contact_info(self, name, email, url=None):
        self.data["contact"] = dict(name=name, email=email)
        if url:
            self.data["url"] = url
    ...
    def add_schema_function(self, swagger_data: Dict[str, Any]):
        ...
        if "decorate" not in target_func.__qualname__:
            ...
            data.update(
                {k: v for k, v in swagger_data.items() if k not in ["function"]}
            )
            ...
            if field_name not in schema.exclude:
                ...
                if field_type in [
                    ...
                    if field_type in [
                        ...
                        pr_field["format"] = (
                            "date" if field_type is fields.Date else "date-time"
                        )
                    ...
                    if field_type is fields.List:
                        if type(field.container) is fields.Nested:
                            # lookup object name
                            field_schema = (
                                field.container.nested.__class__.__name__.replace(
                                    "Schema", ""
                                )
                            )
                            if field_schema not in self.definitions:
                                self.definitions[
                                    field_schema
                                ] = field.container.nested, self.schema_to_definition(
                    ...
                    pr_field = {"type": field_type.__name__}
                    pass
    ...
    def add_path_method(
        ...
        if method not in self.paths[url]:
            ...
            if description is None:
                if obj_string is None:
                    raise NotImplementedError(
                        "Cannot auto-generate description without an input schema or an output "
                        "schema"
                    )
                ...
                if method == "get" and out_schema and many:
                    ...
                    description = (
                        f"Update a {obj_string} based on an id and user supplied input."
                    )
            ...
            responses = {}
            if out_schema:
                swagger_schema = {"$ref": f"{REFERENCE_PATH}{obj_string}"}

                if many:
                    responses[200] = dict(
                        description=f"List of {obj_string}s response",
                        content=dict(
                            {
                                "application/json": dict(
                                    schema={"type": "array", "items": swagger_schema}
                                )
                            }
                        ),
                    )
                    if obj_string not in self.definitions:
                        self.definitions[
                            obj_string
                        ] = out_schema, self.schema_to_definition(
                            obj_string, out_schema
                        )
                else:
                    responses[200] = dict(
                        description=f"{obj_string} response",
                        content=dict({"application/json": dict(schema=swagger_schema)}),
                    )
                ...
                if rule.rule in urls:
                    ...
                    for method in [r.lower() for r in rule.methods]:
                        if method in self.function_map[class_name]:
                            ...
                            if (
                                "out_schema" in api_details
                                and "in_schema" in api_details
                            ):
                                many = (
                                    api_details["detect_many"]
                                    and len(rule.arguments) == 0
                                ) or api_details["many"]
                                path_props = dict(
                                    in_schema=api_details["in_schema"],
    ...
    def generate_swagger_spec(
        ...
        spec.update(
            dict(
                components=dict(schemas={k: v[1] for k, v in self.definitions.items()})
            )
        )
