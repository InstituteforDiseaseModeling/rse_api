...
def default_dramatiq_setup_broker(app):
    ...
    if HAS_DRAMATIQ and (HAS_RABBIT or HAS_REDIS):
        ...
        if (
            ...
            or app.env in ["development", "testing", "documentation"]
        ) and not app.config.get("DRAMATIQ_USE_PROD", False):
            app.logger.info("Using Stub Broker")
            if os.name == "nt":
                # at moment, windows cannot use stub broker
                if not HAS_RABBIT:
                    raise NotImplementedError(
                        "Windows does not support StubBroker. Please use RabbitMQ"
                    )
...
@singleton_function
def get_application(
    setting_object_path: str = None,
    setting_environment_variable: Optional[str] = None,
    ...
    if setting_object_path:
        app.logger.debug(
            "Loading Application settings from {}".format(setting_object_path)
        )
        app.config.from_object(setting_object_path)
    if setting_environment_variable:
        app.logger.debug(
            "Loading Application settings from the file {}".format(
                os.environ[setting_environment_variable]
            )
        )
...
if HAS_APSCHEDULER:
    ...
    from rse_api.decorators import CRON_JOBS, singleton_function
...
def get_worker_cli(
    ...
    if HAS_DRAMATIQ:
        ...
        @click.option(
            "--cron", default=True, help="Whether we want to run cron jobs as well"
        )
        @click.option(
            "--processes", default=None, help="Whether we want to run cron jobs as well"
        )
